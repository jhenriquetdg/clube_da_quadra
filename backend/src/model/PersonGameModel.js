...
async function deletePersonGame(personGame) {
    try {
      var db = await db_open();
      try {
        const personGameResult = await db.get("SELECT * FROM PessoaPartida WHERE Pessoa_CPF=? AND Partida_ID=?", [
            personGame.pessoa_cpf,
            personGame.partida_id
        ]);
        await db.get("DELETE FROM PessoaPartida WHERE Pessoa_CPF=? AND Partida_ID=?", [
            personGame.pessoa_cpf,
            personGame.partida_id
        ]);
        return personGameResult;
