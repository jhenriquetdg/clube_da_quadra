    ...
    try {
        ...
        try {
          const courtResult = await db.run(
            "INSERT INTO Quadra (descricao, endereco) VALUES (?,?)",
            [
                court.descricao,
                court.endereco
            ]
...
async function updateCourt(court) {
    try {
        var db = await db_open();
        try {
            const oldCourt = await db.get("SELECT * FROM Quadra WHERE ID=?", [court.ID]);
            await db.run("UPDATE Quadra SET descricao=?, endereco=? WHERE ID=?", [court.descricao, court.endereco, court.ID]);
            const newCourt = await db.get("SELECT * FROM Quadra WHERE ID=?", [court.ID]);
            return ({
                oldCourt,
                newCourt
            });
